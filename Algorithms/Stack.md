...
| `MULTIPOP(S, k)` | O(min(s, k)) | Извлекает из стека S k верхних объектов; если стек содержит менее k объектов, извлекается всё содержимое |

### Реализации

**Через массив**: массив S[1..n], скалярный атрибут S.top (индекс последнего добавленного элемента). При `push` инкрементируется S.top и записывается значение в S[S.top]. При `pop` декрементируется S.top и возвращается S[S.top + 1]. При фиксированном размере — нужна проверка на overflow. При POP удалённый элемент физически остаётся в ячейке памяти, но логически исключается из множества стека, так как указатель вершины сдвинулся вниз.

**Через связный список**: класс StackNode с полями data и указателем next. Указатель top на голову списка. Добавление и удаление с головы. Не требует заранее заданного размера.
...
| Через linked list | Динамический размер, избегает overflow | Доп. память на указатели и сборку мусора |
...
- **Амортизированный анализ последовательности операций**: цепочку из n операций PUSH, POP, MULTIPOP оценивать не как O(n²) в худшем случае, а как O(n). Элемент извлекается максимум один раз на каждое добавление → амортизированная стоимость любой операции O(1). Метод потенциалов: Φ(Dᵢ) = s (текущий размер стека), амортизированная стоимость PUSH = 2, POP = 0, MULTIPOP = 0.
- **Списки свободных объектов (Free lists)** при выделении памяти работают по принципу стека: последний освобождённый объект становится первым выделяемым при новом запросе.
...
Что такое MULTIPOP(S, k)?::Операция, извлекающая из стека S k верхних объектов. Если стек содержит менее k объектов, извлекается всё содержимое. Стоимость = O(min(s, k)).

Амортизированный анализ стека (n операций PUSH, POP, MULTIPOP)?::Суммарно O(n), т.к. каждый элемент извлекается max 1 раз на каждое добавление. Метод потенциалов: Φ = размер стека. Амортизированная стоимость PUSH = 2, POP = 0, MULTIPOP = 0.
